  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex justify-between max-w-2xl mx-auto px-4">
        {(["discovery", "rights", "signing", "delivery", "audit"] as const).map(
          (s, i) => (
            <div key={s} className="flex flex-col items-center gap-2">
              <div
                className={`h-2 w-12 sm:w-20 rounded-full transition-colors ${
                  stage === s
                    ? "bg-primary shadow-[0_0_10px_rgba(204,21,18,0.5)]"
                    : i <
                        [
                          "discovery",
                          "rights",
      ...
      <div className="max-w-3xl mx-auto space-y-12">
        {stage === "discovery" && (
          <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 space-y-8">
            <article className="postal-card p-8 sm:p-12 space-y-6 shadow-2xl">
              <span className="stamp-badge text-primary italic font-bold">
                1. Rights Discovery
              </span>
              <h2 className="font-headline text-3xl sm:text-5xl font-black uppercase text-cream leading-tight">
                An agent detects <br /> your rights signal.
              </h2>
              <p className="text-muted text-lg leading-relaxed italic">
                The AI agent doesn't just read. It parses machine-readable rules
                in the HTML metadata to detect the rights checkout.
              </p>
              ...
              <button
                ...
                className="stamp-button w-full py-5 text-xl"
              >
                {isProcessing ? "Detecting..." : "Analyze Rights Checkout →"}
              </button>
            </article>
          </div>
        )}
          ...
          <div className="animate-in fade-in slide-in-from-right-6 duration-700 space-y-8">
            <div className="postal-card p-8 sm:p-12 space-y-8 border-gold/20 shadow-2xl">
              ...
              <p className="text-muted text-lg italic leading-relaxed">
                The agent confirms that "Voice Narration" requires an automated
                checkout under the current rules.
              </p>
              <button
                onClick={nextStage}
                disabled={isProcessing}
                className="stamp-button w-full py-5 text-xl"
              >
                {isProcessing ? "Negotiating..." : "Proceed to Settlement →"}
              </button>
            </div>
          </div>
        )}
        ...
        {stage === "signing" && (
          <div className="animate-in fade-in zoom-in-95 duration-700 space-y-8">
            ...
            <div className="postal-card p-8 bg-primary/5 border-primary/20 text-center space-y-6">
              <h3 className="font-headline text-2xl font-bold text-cream uppercase">
                Rights Settlement
              </h3>
              <p className="text-muted italic">
                Simulating a Solana Devnet transaction. The agent is executing
                an x402-style settlement.
              </p>
              <button
                onClick={nextStage}
                disabled={isProcessing}
                className="stamp-button w-full py-6 text-2xl font-black shadow-[0_0_20px_rgba(204,21,18,0.3)]"
              >
                {isProcessing
                  ? "Settling..."
                  : "Settle x402 Payment (0.10 USDC)"}
              </button>
            </div>
          </div>
        )}
        ...
        {stage === "delivery" && (
          <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 space-y-8">
            <div className="postal-card p-8 sm:p-12 text-center space-y-8 border-green-ink/20 shadow-2xl">
              ...
              <h2 className="font-headline text-3xl sm:text-5xl font-black uppercase text-cream leading-tight">
                Settlement Verified. <br />{" "}
                <span className="text-green-ink">Proof of Consent Issued.</span>
              </h2>
              <div className="p-6 border border-green-ink/30 bg-green-ink/5 font-mono text-sm text-green-ink flex items-center justify-between">
                ...
                <span className="stamp-badge border-green-ink/40 text-[9px]">
                  READY
                </span>
              </div>
              <button
                ...
                className="stamp-button w-full py-5 text-xl"
              >
                {isProcessing ? "Finalizing..." : "View Audit Trail →"}
              </button>
            </div>
          </div>
        )}
        ...
        {stage === "audit" && (
          <div className="animate-in fade-in duration-1000 space-y-8">
            ...
            <button
              onClick={nextStage}
              className="stamp-button stamp-button-secondary w-full py-4 uppercase tracking-widest text-xs"
            >
              Restart Simulation
            </button>
          </div>

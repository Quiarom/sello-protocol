...
function PermissionRow({
  ...
  return (
    <div className="flex items-start justify-between gap-3 border-b border-border-low py-2 text-sm">
      <span className={`pr-3 ${value ? "" : "text-muted line-through"}`}>
        {label}
      </span>
...
export function LicenseResultCard({
  ...
  const protectionLevel = Math.max(
    0,
    Math.min(100, (result.complianceScore / 4) * 100)
  );
    ...
    <div className="postal-card p-6 sm:p-10 space-y-8 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-6 border-b border-border-low pb-8">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted max-w-sm italic">
            Machine-readable rights verified via Sello Protocol on-chain notary.
          </p>
        </div>
          ...
          {result.onchainVerified && (
            <div className="flex items-center gap-2 bg-green-ink/10 text-green-ink border border-green-ink/30 px-3 py-1 text-xs uppercase font-mono font-bold tracking-widest">
              <svg className="h-3 w-3" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
                  d="M2.166 4.9L9.03 9.069a2.25 20 0 002.24 0l6.864-4.171A2.25 2.25 0 0015.81 2H4.19a2.25 2.25 0 00-2.024 2.9zM18 8.162l-6.685 4.061a3.75 3.75 0 01-2.63 0L2 8.162V15.75A2.25 2.25 0 004.25 18h11.5A2.25 2.25 0 0018 15.75V8.162z"
                  clipRule="evenodd"
                />
              </svg>
              On-Chain Verified
      ...
      <div className="grid gap-10 md:grid-cols-2">
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-background/80 border border-border-low p-4 relative group">
              <code className="block break-words text-xs leading-relaxed text-muted-foreground">
                {result.attribution}
              </code>
              <button
                onClick={() => {
          ...
          {explorerPdaUrl && (
            <div className="space-y-2">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex max-w-full items-center gap-2 break-all text-xs font-mono text-primary hover:underline group"
              >
        ...
        <div className="space-y-6">
          ...
          <div className="divide-y divide-border-low border-t border-border-low">
            <PermissionRow
              label="Inference / Summarize"
              value={result.permissions.canSummarize}
            />
            <PermissionRow
              label="Contextual Quotes"
              value={result.permissions.canQuote}
            />
            <PermissionRow
              label="Voice Narration"
              value={result.permissions.canVoice}
              priceUSDC={result.priceUSDC}
            />
            <PermissionRow
              label="Model Training"
              value={result.permissions.canTrain}
            />
          </div>

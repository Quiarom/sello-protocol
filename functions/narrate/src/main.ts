...
export default async ({ req, res, log, error }: any) => {
  ...
  try {
    ...
    if (cached) {
      ...
      return respondWithAudio(res, null, fileUrl, sello_pda, log);
    }
  } catch {
    // cache miss, continue
  }
  ...
  return respondWithAudio(res, audioBuffer, audioUrl, sello_pda, log);
};
...
function respondWithAudio(
  ...
  if (audioUrl && !audioBuffer) {
    ...
    return res.json({ audioUrl, selloPda: selloPda ?? null }, 200, {
      "Access-Control-Allow-Origin": "*",
    });
  }
  if (audioBuffer) {
